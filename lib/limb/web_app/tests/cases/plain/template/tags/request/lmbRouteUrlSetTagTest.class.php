 ...
 * @version    $Id: lmbRouteUrlSetTagTest.class.php 5012 2007-02-08 15:38:06Z pachanga $
 * @package    web_app
 */
...
class lmbRouteUrlSetTagTest extends lmbWactTestCase
{
  function testPutUrlToCurrentDataspaceAllParamsAreStaticAndUseNamedRoute()
  {
    ...
    $expected = $routes->toUrl(array('controller' => 'news', 'action' => 'archive'), 'news');
  ...
  function testWithDynamicParams()
  {
    ...
    $expected = $routes->toUrl(array('controller' => $controller, 'action' => $action), 'news');
    $this->assertEqual($page->capture(), $expected);
  }
  ...
  function testTryToGuessRoute()
  {
    ...
    $expected = $routes->toUrl(array('controller' => 'news', 'action' => 'archive'));
    $this->assertEqual($page->capture(), $expected);
  }
