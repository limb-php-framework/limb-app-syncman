 ...
 * @version    $Id: WactTagAttributesTest.class.php 5021 2007-02-12 13:04:07Z pachanga $
 * @package    wact
 */
...
class WactTagAttributesTest extends WactTemplateTestCase
{
  ...
  function testMinimizedAttribute() {
      ...
      $this->registerTestingTemplate('/attributes/minimizedattribute.html', $template);
      $page = $this->initTemplate('/attributes/minimizedattribute.html');

      $form = $page->getChild('test');
      $this->assertNull($form->getAttribute('extra'));

      $output = $page->capture();
      $this->assertEqual($output, '<P id="test" extra>contents</P>');
  }
